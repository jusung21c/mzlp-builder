...
export default class SelectLangView{
  ...
  serialize() {
    this.langClickListener(this._builder)
  }
  ...
  langClickListener(builder){
      ...
      $("#btn_lang_OK").click(function(){
          for(let i in this._langlist){
            console.log(this._langlist[i])
              if($('#sel_lang_'+this._langlist[i]).is(":checked")){
                  alert(this._langlist[i]+"is checked!");
                  selectedLang.push(this._langlist[i]);
                  //해당 빌더 실행 펑션 인자값으로
              }
          };
